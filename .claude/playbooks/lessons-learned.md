...
## Session 2026-04-18 — M5/M6 + test audit

### "Multi-jump captures reveal bugs in PDN writers years later"
`king_blunder_bug.pdn` — user's game replay showed AI hanging a king.
Root cause was NOT the AI: `_infer_pdn_move_from_boards` inferred the
mover colour by counting vanished pieces (treating every captured
piece as a source candidate). When captures outnumbered real sources
(3 captures vs 2 pieces moved) the heuristic flipped, storing the
wrong move and misaligning the variation tree. Ply 17 looked like
ply 18 — the king "blundered" because the wrong side was to move.
- **Rule:** mover colour comes from the *destination* piece, not the
  set of vanished pieces. The destination square is always unambiguous.
- **Detection:** round-trip test every multi-jump capture through
  write-then-read-then-replay, comparing full capture paths (not just
  source/dest).
- Fixed by `_infer_pdn_move_from_boards` rewrite + 10 regression tests
  in `test_pdn_inference_roundtrip.py`.

### "Retrograde analysis needs all material splits, including lopsided"
3-piece bitbase enumerator only generated balanced splits like 2B+1W
and 1B+2W — never 2B+0W or 0B+2W. But retrograde propagation walks
*backwards* from terminal states; when a capture in a 2B+1W position
leaves 2B+0W, that child state has no entry. Propagation stalled
silently — some positions stayed DRAW because their children were
never visited. Root cause of a non-trivial set of "drawn position
played as win" incidents.
- **Rule:** enumerate EVERY possible material split that the piece
  budget allows, including ones where one side has zero pieces.
- **Detection:** for each (color, remaining_kind) pair, emit the
  single-side-only split and verify it appears in the output keyspace.
- Fixed via `_enumerate_lopsided_same_side` + regression tests in
  `test_bitbase_enumeration.py`. 4-piece bitbase has the same bug —
  regenerate before shipping.

### "Bitbase tie-break must prefer material-preserving wins"
`_bitbase_best_move` iterated candidates and kept the first WIN. Two
winning king moves: sacrifice a king for faster mate vs. preserve
material for slower mate — the engine chose arbitrarily. In 2K vs 1K
this produced the visible "king walks into a pawn" blunder even
though the position is theoretically winning.
- **Rule:** among moves with identical bitbase-score, break ties by
  static eval (preserve material, shorten the conversion path).
- Fixed via tuple compare `(score, eval)` + regression guards in
  `test_bitbase_tiebreak.py`.

### "Parametrise to kill test splinter patterns"
Pre-audit: `test_themes.py` had 17 near-identical tests, one per
theme × one per cell-type; `test_ux_polish.py` had 22 tautological
tests against a `_FakeBoardWidget` that never exercised real code.
Audit outcome:
- `test_themes.py`: 17 → 10 via `@pytest.mark.parametrize`
- `test_ux_polish.py`: 22 → 4 against real BoardWidget
- `test_board_editor.py`: `TestEditorModeToggle` 3→1
- Net: -30 tests, same coverage, higher signal density.
- **Rule:** if you see N tests differing only in a single literal,
  parametrise. If a test uses a `_Fake*` that mirrors production
  interface with no logic, it's a tautology — delete and write one
  real test against the production type.

### "Tests that enforce unverified defensive code pay double"
During Phase-A rules-gap audit, `test_kings_only_rule_does_not_fire_with_pawns`
was added expecting the rule to be pawn-aware. It failed — revealing
that production `check_game_over` never checked for pawns. The test
doubled as bug-finder AND regression guard. Budget time for gap-tests
that *assert the behaviour you wish existed*, not just the behaviour
you know exists.

### "pytest markers belong in pyproject.toml, not folk tradition"
Unregistered markers (`@pytest.mark.tactics`, `@pytest.mark.slow`)
produce `PytestUnknownMarkWarning` for every test run and silently
allow typos (`@pytest.mark.slwo` becomes a no-op selector). Register
each marker in `[tool.pytest.ini_options].markers` with a one-line
purpose comment. Now `pytest -m "not slow"` and `pytest -m tactics`
are first-class selectors, documented in one place.

### "Canonical user-data paths prevent path sprawl"
Before: bitbase at `%APPDATA%/DRAUGHTS/bitbase.json`, mined puzzles at
`draughts/resources/mined_puzzles.json` (inside the package!), opening
book at a third location. Three sinks, three migration stories, three
bug surfaces. Consolidated into `draughts/user_data.py` with
`bitbase_dir()` / `books_dir()` / `puzzles_dir()` + a one-shot
`migrate_legacy_paths()`. Any new data artefact goes through this
module — prevents future drift.
