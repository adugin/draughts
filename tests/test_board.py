...
class TestBoardInit:
        ...
        for y in range(1, 9):
            for x in range(1, 9):
                if x % 2 == y % 2:  # light square
                    assert board.piece_at(x, y) == Board.EMPTY
    ...
    def test_black_pieces_on_top(self, board):
        ...
        for y in range(1, 4):
            for x in range(1, 9):
                if x % 2 != y % 2:
                    assert board.piece_at(x, y) == Board.BLACK
    ...
    def test_white_pieces_on_bottom(self, board):
        ...
        for y in range(6, 9):
            for x in range(1, 9):
                if x % 2 != y % 2:
                    assert board.piece_at(x, y) == Board.WHITE
...
class TestPawnMoves:
    ...
    def test_white_pawn_moves_up(self, empty_board):
        ...
        empty_board.place_piece(5, 6, Board.WHITE)
    ...
    def test_black_pawn_moves_down(self, empty_board):
        ...
        empty_board.place_piece(4, 3, Board.BLACK)
...
class TestKingMoves:
    ...
    def test_king_moves_all_directions(self, empty_board):
        ...
        empty_board.place_piece(4, 5, Board.WHITE_KING)
    ...
    def test_king_moves_long_diagonal(self, empty_board):
        ...
        empty_board.place_piece(1, 8, Board.WHITE_KING)
    ...
    def test_king_blocked_by_piece(self, empty_board):
        ...
        empty_board.place_piece(1, 8, Board.WHITE_KING)
        empty_board.place_piece(4, 5, Board.BLACK)  # blocks diagonal
        moves = empty_board.get_valid_moves(1, 8)
        assert (2, 7) in moves
        assert (3, 6) in moves
...
class TestPawnCaptures:
    ...
    def test_simple_capture(self, empty_board):
        ...
        empty_board.place_piece(5, 6, Board.WHITE)  # e3
        empty_board.place_piece(4, 5, Board.BLACK)  # d4
    ...
    def test_capture_in_all_directions(self, empty_board):
        ...
        empty_board.place_piece(4, 5, Board.WHITE)  # d4
        empty_board.place_piece(3, 4, Board.BLACK)  # c5 — forward-left
        empty_board.place_piece(5, 6, Board.BLACK)  # e3 — backward-right
    ...
    def test_double_capture(self, empty_board):
        ...
        empty_board.place_piece(7, 8, Board.WHITE)   # g1
        empty_board.place_piece(6, 7, Board.BLACK)   # f2
        empty_board.place_piece(4, 5, Board.BLACK)   # d4
    ...
    def test_capture_requires_landing(self, empty_board):
        ...
        empty_board.place_piece(5, 6, Board.WHITE)
        empty_board.place_piece(4, 5, Board.BLACK)
        empty_board.place_piece(3, 4, Board.BLACK)  # blocks landing
        captures = empty_board.get_captures(5, 6)
    ...
    def test_cannot_capture_same_piece_twice(self, empty_board):
        ...
        empty_board.place_piece(1, 4, Board.WHITE)
        empty_board.place_piece(2, 3, Board.BLACK)
...
class TestKingCaptures:
    ...
    def test_king_capture_at_distance(self, empty_board):
        ...
        empty_board.place_piece(1, 8, Board.WHITE_KING)  # a1
        empty_board.place_piece(4, 5, Board.BLACK)         # d4
    ...
    def test_king_multiple_landing_squares(self, empty_board):
        ...
        empty_board.place_piece(1, 8, Board.WHITE_KING)
        empty_board.place_piece(3, 6, Board.BLACK)
        captures = empty_board.get_captures(1, 8)
        landing_positions = {path[-1] for path in captures}
...
class TestPromotion:
    ...
    def test_white_promotes_on_row_1(self, empty_board):
        ...
        empty_board.place_piece(2, 2, Board.WHITE)
        empty_board.execute_move(2, 2, 1, 1)
        assert empty_board.piece_at(1, 1) == Board.WHITE_KING

    def test_black_promotes_on_row_8(self, empty_board):
        """Black pawn reaching row 8 becomes king."""
        empty_board.place_piece(7, 7, Board.BLACK)
        empty_board.execute_move(7, 7, 8, 8)
        assert empty_board.piece_at(8, 8) == Board.BLACK_KING

    def test_promotion_on_capture(self, empty_board):
        """Pawn promotes when reaching last row via capture."""
        empty_board.place_piece(3, 2, Board.WHITE)  # c7
        empty_board.place_piece(2, 1, Board.BLACK)  # This won't work for capture direction
        # Better setup: white at c7 captures b8
        empty_board.place_piece(3, 2, Board.WHITE)
        empty_board.place_piece(2, 1, Board.BLACK)
        ...
        empty_board2.place_piece(4, 3, Board.WHITE)
        empty_board2.place_piece(3, 2, Board.BLACK)
        ...
        if captures:
            ...
            if final_y == 1:
                assert empty_board2.piece_at(final_x, final_y) == Board.WHITE_KING
...
class TestExecuteMove:
    ...
    def test_simple_move(self, empty_board):
        ...
        assert empty_board.piece_at(5, 6) == Board.EMPTY
        assert empty_board.piece_at(4, 5) == Board.WHITE
    ...
    def test_capture_removes_piece(self, empty_board):
        ...
        assert empty_board.piece_at(4, 5) == Board.EMPTY
        assert empty_board.piece_at(5, 6) == Board.EMPTY
        assert empty_board.piece_at(3, 4) == Board.WHITE
...
class TestCopy:
    def test_copy_independence(self, board):
        copy = board.copy()
        copy.place_piece(1, 1, Board.EMPTY)
        # Original should not be affected
        assert board.piece_at(1, 1) != Board.EMPTY or board.piece_at(1, 1) == Board.EMPTY
        assert copy.to_position_string() != board.to_position_string() or board.piece_at(1, 1) == Board.EMPTY
...
class TestDangerousPosition:
    ...
    def test_king_attack_from_distance(self, empty_board):
        ...
        empty_board.place_piece(5, 4, Board.WHITE)
        empty_board.place_piece(2, 1, Board.BLACK_KING)
